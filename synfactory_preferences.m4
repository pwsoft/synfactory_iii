...
define([PrefColorSelector],[
	EN(StringToLangId($2), [$2])
	DefSetVar([color_t], [$3], [$4])
	MacroBack([__preferences_refresh], [[preferencesDrawHeader(aContext, x, y, ]StringToLangId($2)[);]])
	MacroBack([__preferences_refresh], [[preferencesDrawColorBar(aContext, x, y, $3);]])
	MacroBack([__preferences_click], [[preferencesClickColorBar(aContext, x, y, $3, __set_$3);]])
	SynFileHex($1, $3);
])
...
static const int colorSelectorSteps = (int)(sizeof(colorSelector[0])/sizeof(colorSelector[0][0]));
static void preferencesDrawColorBar(Context_ptr_t aContext, int x, int &y, color_t current) {
	guiSelectPenColor(aContext, -1, 0);
	for(int i=0;i<3;i++) {
		for(int j=0;j<colorSelectorSteps;j++) {
...
static bool preferencesClickColorBar(Context_ptr_t aContext, int x, int &y, color_t &value, SetColorCallback_t setFunction) {
	...
	for(int color=0; color<3; color++) {
		if (aContext->mouseX >= x+30 && aContext->mouseX < x+30+colorSelectorSteps*14
