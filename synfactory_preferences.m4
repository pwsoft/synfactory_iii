...
define([PrefColorSelector],[
	...
	MacroBack([__preferences_refresh], [[configDrawHeader(aContext, x, y, $1);]])
	MacroBack([__preferences_refresh], [[configDrawColorBar(aContext, x, y, $2);]])
	MacroBack([__preferences_click], [[y += 14;]])
	MacroBack([__preferences_click],
[[for(int color=0; color<3; color++) {
	if (aContext->mouseX >= 30 && aContext->mouseX < 30+colorSelectorSteps*14
	&& aContext->mouseY >= y && aContext->mouseY < y+20) {
		$2 = ($2 & colorNotMask[color]) | colorSelector[color][(aContext->mouseX-30)/14];
